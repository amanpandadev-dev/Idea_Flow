...
import { KeyRound, Lock, User, Mail, ArrowLeft, Loader2, CheckCircle, AlertCircle } from 'lucide-react';
...
const ForgotPasswordPage: React.FC<ForgotPasswordPageProps> = ({ onNavigateToLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await resetPassword(formData.emp_id, formData.email, formData.newPassword);
      setSuccess(true);
    } catch (err: any) {
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-xl shadow-xl p-8 max-w-md w-full text-center">
          ...
          <p className="text-slate-500 mb-6">Your password has been updated. You can now login with your new credentials.</p>
          <button 
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 bg-[url('https://images.unsplash.com/photo-1451187580459-43490279c0fa?auto=format&fit=crop&q=80')] bg-cover bg-center bg-no-repeat relative">
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        <div className="bg-white/95 backdrop-blur-md py-8 px-4 shadow-2xl sm:rounded-xl sm:px-10 border border-white/20">
          ...
          <div className="mt-6 text-center">
            <button 
               onClick={onNavigateToLogin} 
               className="flex items-center justify-center gap-2 w-full text-sm font-medium text-slate-500 hover:text-slate-800 transition-colors"
            >
