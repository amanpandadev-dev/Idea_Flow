...
import { generateGeminiEmbeddingWithRetry, isGeminiAvailable } from '../config/gemini.js';
 ...
 * @param {'gemini' | 'llama' | 'grok'} [provider] - The embedding provider to use. Defaults to EMBEDDING_PROVIDER from .env.
 * @returns {Promise<number[]>} The embedding vector.
 */
export async function getEmbeddingVector(text, provider) {
  const selectedProvider = provider || process.env.EMBEDDING_PROVIDER || 'gemini';
    ...
    case 'gemini':
      try {
        ...
        if (!isGeminiAvailable()) {
          console.warn(`⚠️  Gemini not available, falling back to Ollama (llama)...`);
          return generateOllamaEmbedding(text);
        }
        return await generateGeminiEmbeddingWithRetry(text);
      } catch (error) {
        // Fall back to Ollama on any Gemini error
        console.warn(`⚠️  Gemini embedding failed, falling back to Ollama (llama)...`);
